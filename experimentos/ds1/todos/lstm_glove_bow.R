...
files <- c("experimentos/ds1/q1/lstm/glove_bow_q1.R")

for (file in files) {
	redeDesc <- "LSTMGloVeBowDS1Q1"
	for (epoca in epocas) {
...
files <- c("experimentos/ds1/q2/lstm/glove_bow_q2.R")

for (file in files) {
	redeDesc <- "LSTMGloVeBowDS1Q2"
	for (epoca in epocas) {
		for (metrica in metricas) {
			for (enriquecimento in enriquecimentos) {
				resultados <- data.frame(matrix(ncol = 4, nrow = 0))
...
files <- c("experimentos/ds1/q3/lstm/glove_bow_q3.R")

for (file in files) {
	redeDesc <- "LSTMGloVeBowDS1Q3"
	for (epoca in epocas) {
		for (metrica in metricas) {
			for (enriquecimento in enriquecimentos) {
				resultados <- data.frame(matrix(ncol = 4, nrow = 0))
