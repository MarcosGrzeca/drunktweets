...
for (year in 1:20) {
	...
	entities_out <- auxiliary_input %>%
                layer_dense(units = 4, activation = 'relu', name = "ReLuEntidades")

	auxiliary_input_types <- layer_input(shape = c(max_sequence_types))
	types_out <- auxiliary_input_types %>%
                layer_dense(units = 4, activation = 'relu', name = "ReLuTypes")
	...
	if (enriquecimento == 1) {
		main_output <- layer_concatenate(c(flatten_out, entities_out, types_out)) %>% 
				layer_dropout(0.2) %>%
