...
w2v <- readr::read_delim("fasttext/fast_text_result.txt", 
                  skip=1, delim=" ", quote="",
...
for (iteracao in 1:10) {
  ...
  X <- as(cbind(embed, pca_entities$x[,1:13]), "dgCMatrix")
  ...
  # out-of-sample accuracy
  preds <- predict(rf, X[test,])
  resultados <- addRowSimple(resultados, "PCA", round(precision(preds>.50, dados$resposta[test]) * 100,6), round(recall(preds>.50, dados$resposta[test]) * 100,6))
