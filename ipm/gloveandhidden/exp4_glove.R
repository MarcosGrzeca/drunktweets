...
for (year in 1:10){
  try({
    ...
    relu <- main_input %>% 
      layer_embedding(vocab_size, embedding_dims, input_length = maxlen, name="layerEmbedding") %>%
      layer_dropout(0.1) %>%
      layer_conv_1d(
        filters, kernel_size,
    ...
    relu_hidden_input <- layer_input(shape = c(maxlen), dtype = "int32")
    relu_hidden <- relu_hidden_input %>% 
    ...
    main_output <- layer_concatenate(c(relu, entities_out, types_out, relu_hidden)) %>%  
    ...
    model <- keras_model(
      inputs = c(main_input, auxiliary_input, auxiliary_input_types, relu_hidden_input),
      outputs = main_output
    )
    
    get_layer(model, name = "layerEmbedding") %>%
    ...
    history <- model %>%
      fit(
        x = list(dados_train_sequence, train_sequences, train_sequences_types, dados_train_sequence),
    ...
    # history
    predictions <- model %>% predict(list(dados_test_sequence, test_sequences, test_sequences_types, dados_test_sequence))
