		...
		for (year in 1:10) {
			try({
			    ...
			    if (enriquecimento == 1) {
				    ...
				    # history
				    predictions <- model %>% predict(dados_test_sequence)
				}
			    ...
			    if (expName == "Exp5") {
					if (enriquecimento == 1) {
						...
						saveRDS(resultados, file = "ipm/expsperdidos/resultadosautomatico/resultados_exp5_glove_sem_enriquecidos_2.rds")
					}
				}
		  })
		}
