...
main_output <- layer_maximum(c(ccn_out, entities_out, types_out)) %>%  
  layer_dense(units = 64, activation = 'relu') %>% 
...
adicionarResultadosTestes("Maximum", matriz$byClass["F1"] * 100, matriz$byClass["Precision"] * 100, matriz$byClass["Recall"] * 100)
