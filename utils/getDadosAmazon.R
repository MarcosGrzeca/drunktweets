...
getDadosAmazon <- function() {
  dados <- query('SELECT id,
                      ...
                      textEmbedding,
                      (
                        SELECT GROUP_CONCAT(CONCAT("type", DISTINCT(REPLACE(ty.type, "http://dbpedia.org/class/", ""))))
                      ...
                      (
                        SELECT GROUP_CONCAT(CONCAT("entidade", DISTINCT(REPLACE(tn.palavra, ",", ""))))
                        FROM tweets_amazon_nlp tn
                        WHERE tn.idTweet = t.id
                  ...
                  SELECT id,
                      ...
                      textEmbedding,
                      (
                      SELECT GROUP_CONCAT(CONCAT("type", DISTINCT(REPLACE(type, "http://dbpedia.org/class/", ""))))
                      FROM
                      ...
                      (
                      SELECT GROUP_CONCAT(CONCAT("entidade", REPLACE(tn.palavra, ",", "")))
                      FROM tweets_nlp tn
