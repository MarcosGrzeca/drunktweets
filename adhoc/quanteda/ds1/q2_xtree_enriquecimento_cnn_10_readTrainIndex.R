...
# for (iteracao in 1:10) {
  ...
  trainingRead <- readRDS(file = paste0("ensemble/resample/exp2/trainIndex1.rds"))
  View(sample(1:nrow(dados), floor(.80 * nrow(dados))))
  
  marcos <- as.data.frame(trainingRead)
  trainig2 <- marcos$Resample1
  View(marcos$Resample1)
