...
for (i in 1:10) {
  ...
  inTrain <- createDataPartition(y = X[, 151], p = split, list = FALSE)
  
  one_hot_train <- X[inTrain, 1:150]
  resposta <-  X[inTrain, 151]
  
  one_hot_test <- X[-inTrain, 1:150]
  resposta_test <-  X[-inTrain, 151]
  
  # MODEL --------------------------------------------------------------
  model <- keras_model_sequential() %>%
    layer_dense(units = 128, activation = "relu", input_shape = c(150)) %>%
