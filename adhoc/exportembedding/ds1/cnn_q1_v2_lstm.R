...
lstm <- embedding_input %>%
        layer_lstm(units = 164, return_sequences = TRUE) %>%
        layer_lstm(units = 128, return_sequences = TRUE, recurrent_dropout = 0.2) %>%
        layer_lstm(units = 128)

main_output <- layer_concatenate(c(ccn_out_3, ccn_out_4, ccn_out_5, lstm)) %>% 
  layer_dropout(0.2) %>%
...
embedding_file <- "adhoc/exportembedding/ds1/q1/100_lstm_cnn_10_epocas_8_filters164.txt"
