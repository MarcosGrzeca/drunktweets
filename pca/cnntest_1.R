...
for (year in 1:20) {
	...
	filters_auxiliary <- 30
	auxiliary_ccn_out_3 <- 	auxilary_output %>% 
							layer_conv_1d(
								filters_auxiliary, 3,
								padding = "valid", activation = "relu", strides = 1
							) %>%
	...
	auxiliary_ccn_out_4 <- 	auxilary_output %>% 
							layer_conv_1d(
								filters_auxiliary, 4, 
								padding = "valid", activation = "relu", strides = 1
							) %>%
	...
	auxiliary_ccn_out_5 <- 	embedding_input %>% 
							layer_conv_1d(
								filters_auxiliary, 5, 
								padding = "valid", activation = "relu", strides = 1
							) %>%
	...
	cnn_output <- 	layer_concatenate(c(auxiliary_ccn_out_3, auxiliary_ccn_out_4, auxiliary_ccn_out_5)) %>% 
					layer_dropout(0.2) %>%
					layer_dense(units = 8, activation = "relu", kernel_regularizer = regularizer_l2(0.001))

	main_output <- 	layer_concatenate(c(cnn_output, cnn_output)) %>% 
